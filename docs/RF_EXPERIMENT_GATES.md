...
| Selective dual-IF spur confirmation | Official base/alternate/full-dual hardware survey complete; no candidate integration | Put real tones on every dynamic/static table boundary over RBW and path; compare `AUTO`, `OFF`, and `ON` amplitude, SFDR, and sweep time | `AUTO` may dual-read only `F_AT_SPUR` low-input bins; preserve full `ON`, deterministic self-tests, and real signals present under both IF choices |
